...
export class AddBookComponent {
  ...
  AddBook(membershipdata) {
    this.http.post("http://localhost:3000/book/" + this.registrationForm.value.author_id + '/' + this.registrationForm.value.genre_id + '/' + this.registrationForm.value.publisher_id, membershipdata).subscribe((response) => { this.showSuccess(response), this._location.back() }, error => { this.errorSuccess() });
