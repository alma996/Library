...
export class EditLoansComponent {
  ...
  EditLoans(selectedItem: true) {
    ...
    if (this.loans_date !== '0000-00-00' && this.loans_date !== null && this.return_status !== null && this.registrationForm.value.return_status !== 0) {
      return this.http.put("http://localhost:3000/loans/" + this.loans_id, selectedItem).subscribe(response => { this.showSuccess(response), this._location.back() }, error => { this.errorSuccess() });
    } else {
      this.errorSuccess()
    }
  }
