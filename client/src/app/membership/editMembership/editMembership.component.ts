...
export class EditMembershipComponent {
  ...
  EditMembership(selectedItem: true) {
    ...
    if (this.date_of_payment !== '0000-00-00' && this.date_of_payment !== null && this.year !== null && this.year !== 0 && this.amount !== 0 && this.amount !== null) {
      return this.http.put("http://localhost:3000/membership/" + this.membership_id, selectedItem).subscribe(response => { this.showSuccess(response), this._location.back() }, error => { this.errorSuccess() });
    } else {
      this.errorSuccess()
    }
  }
